...
export class dal implements nodulus.IDal {
    ...
    connect(callback: Function) {
        if (!this.db || this.db === null) {
            ...
            db.listCollections = function (result: any) {
                var tableList = db.tableList();
                tableList.toArray = function (callback: Function) {
                    tableList.run(function (err: any, results: any) {
                        callback(null, results.map(function (item: string) { return { name: item } }))
                    })
